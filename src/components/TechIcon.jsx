...
const TechIcon = ({ projectName, bg }) => {
	...
	const handleClick = () => {
		...
		setTimeout(
			...
			2000
		); // Kembali ke false setelah 3 detik
	};
	...
	return (
		...
		>
			...
			<span
				className={`logo__wrp not-odd:inline-block transation-all duration-300 delay-75 ${
					!isActive
						? "text-zinc-400"
						: "text-orange-500"
				} `}
			...
			{project.techs.map(
				(
					...
					<span
						...
						className={`logo__wrp opacity-0 whitespace-nowrap overflow-hidden max-w-0 transition-all duration-300 delay-250 md:delay-0  ${
							isActive
