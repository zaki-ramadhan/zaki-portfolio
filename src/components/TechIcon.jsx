...
const TechIcon = ({projectName, bg}) => {
    ...
    return (
        <span id="technologies" onClick={handleClick} className={`group overflow-visible max-w-fit p-2 md:p-2.5 pb-0.5 md:pb-1 flex items-center rounded-xl ${bg} border border-secondary/10 hover:border-secondary/20 backdrop-blur-xs hover:shadow-lg hover:shadow-black/2 active:shadow-lg active:shadow-black/2 absolute bottom-1 md:bottom-4 delay-0 ${isActive ? "bottom-4" : ""} right-1/2 translate-x-1/2 transition-all duration-300 overflow-hidden z-20 scale-90 sm:scale-100`} >
        ...
        <span className="logo__wrp not-odd:inline-block text-orange-500">
         ...
         {/* Ikon lainnya (muncul saat hover) */}
            {project.techs.map((item, index) => (
                <span key={index} className={`logo__wrp opacity-0 whitespace-nowrap overflow-hidden max-w-0 transition-all duration-300 delay-300 md:delay-0  ${isActive ? "opacity-100 max-w-[28px] ms-4" : ""}`} >
