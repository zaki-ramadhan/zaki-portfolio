...
const Cards = () => {
    return (
        <main id="cards" className="container py-4 px-20 flex flex-col -space-y-3">
            ...
            {/* hello card */}
                <Card typeCard={'hello-card'} setBgColor = 'bg-linear-340/srgb from-additional from-[-110%] to-primary to-55%' className={'justify-between rounded-ee-lg basis-3/4'}> 
                    ...
                    <span className="flex items-center gap-1 text-lg font-semibold text-secondary">
                        <Icon icon="famicons:logo-html5" width="28" height="28" className="p-1.5 bg-secondary/10 hover:text-orange-500 transition-all duration-150 rounded-full" />
                        <Icon icon="ion:logo-css3" width="28" height="28" className="p-1.5 bg-secondary/10 hover:text-blue-500 transition-all duration-150 rounded-full" />
                        <Icon icon="mdi:language-javascript" width="28" height="28" className="p-1.5 bg-secondary/10 hover:text-yellow-400 transition-all duration-150 rounded-full" />
                        <span className="ms-1">+3</span>

                    </span>
                </Card>

                <Card is_image className={'card group basis-1/4 min-w-90 rounded-es-lg border border-secondary/15 overflow-hidden'}>
                    <img src={avatarAi} className="h-full group-hover:scale-105 transition-all duration-500"/>
                </Card>
            </div>

            <section id="projects" className="card__wrp container grid grid-cols-2 py-4 mt-4 gap-4">
