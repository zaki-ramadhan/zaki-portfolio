      ...
      <div className="w-full h-fit">
        <div className="w-full title-status__wrp flex items-center justify-between gap-4">
          <span className="title-icon__wrp flex items-center gap-2 basis-3/4 min-w-0">
            <h1 
              className={`${titleColor} text-[25.5px] md:text-3xl font-Archivo max-w-full md:max-w-none overflow-hidden text-ellipsis whitespace-nowrap min-w-0`}
              style={{ flexShrink: 1 }}
            >
              {project.name}
            </h1>{" "}
          ...
          <span
            id="status-project"
            className={`status basis-1/4 max-w-fit min-w-0 inline-block truncate ${titleColor} py-1.5 px-3 rounded-full ${btnColor} text-sm ring-[0.8px] ring-secondary/30 select-none font-bold`}
          >
            {projectStatus}
          </span>
        </div>
