...
const Card = ({ typeCard, setBgColor, titleColor, btnColor, is_image, children, className, is_projectCard, linkProject, projectTitle,  projectDesc, projectStatus, projectPreview, xPosition, bg, overlayColor }) => {
    ...
    const projectCard = is_projectCard ? (
        <>
            <div className={`card relative ${typeCard} h-128 ${setBgColor} ${!is_image ? 'p-10' : ''} ${className} rounded-[2.7rem] flex flex-col border border-additional/10 overflow-hidden`}>
                ...
                <div className={`overlay__layer w-115 h-32 absolute bottom-0 left-1/2 -translate-x-1/2 bg-linear-0 ${overlayColor} from-10%`}></div>
            </div>
        </>
